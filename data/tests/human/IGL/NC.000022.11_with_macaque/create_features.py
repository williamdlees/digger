...
for ref_result in ref_results:
    ...
    for dig_result in digger_results:
        ...
        # find the best match in the vicinity to tune the start position

        best_allele_score = 0.0

        if abs(dig_result['start'] - ref_start) < 20000 \
                and ref_result['CH17 allele'].replace('D', '') == dig_result['imgt_match'].replace('D', '')\
                and float(dig_result['imgt_score']) > best_allele_score:
            res['start'] = dig_result['start']
            res['end'] = dig_result['end']
            res['digger_coord_match'] = 'T'
            best_allele_score = float(dig_result['imgt_score'])
